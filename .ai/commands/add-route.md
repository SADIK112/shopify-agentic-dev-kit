...
1. **Search first.** Check `app/routes/` for an existing route that covers this
   ...
   follow the `create-service` steps first (`.ai/commands/create-service.md`).
...
3. **Create** `app/routes/<name>.tsx` from `.ai/templates/route.tsx.tmpl`.
   The route must:
...
4. **Verify Polaris components** for any route rendering a Polaris UI. Component
   names and props must be confirmed — they change across versions. Read
   `guardrails/shopify-app-home.md` and verify against `shopify.dev`. On Claude
   Code: invoke the `shopify-polaris-app-home` skill for live schema lookup.

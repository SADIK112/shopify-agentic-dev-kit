...
the AGENTS.md loop (Verify Shopify surfaces). Returns a verified brief the
coding agent can rely on. **Never** writes production code or architectural
decisions.
...
2. **Verify every field, mutation, and query** the task requires: read the
   guardrail, then cross-check against `shopify.dev`. On Claude Code: use the
   skill's `search_docs` for live schema lookup. Anything that cannot be
   confirmed from either source is flagged `UNVERIFIED`. The coding agent must
   not ship unverified fields — ever.

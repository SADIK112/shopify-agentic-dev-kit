...
regression test." This is tighter than the `debug` command — use it when the
symptom is known and you need to get to a merged fix fast.
...
- Shopify deprecated API field → verify replacement via `guardrails/shopify-admin.md` and `shopify.dev`; on Claude Code also use the skill's schema search. Update op and re-run codegen.
...
- If it reveals a missing architecture rule:
  ...
  ```
- Follow the **`checkpoint`** command (`.ai/commands/checkpoint.md`) to sync all tracking files and write the session log entry.

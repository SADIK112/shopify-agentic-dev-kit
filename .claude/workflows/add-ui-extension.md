...
Invoke the matching skill now. Confirm the extension **target** (e.g.
`purchase.checkout.block.render`) — agents routinely hallucinate target strings.
...
- Component names and props must be verified against the skill's current library.
  Polaris extension components change across versions; do not guess prop names.
...
- Configuration that the extension reads from the app should flow through
  metafields (see `shopify-custom-data` skill). Extensions cannot reach Prisma.
...
```bash
# Run the skill's validate command on the extension source.
shopify app deploy --only-extensions   # preview on dev store before full deploy
```
